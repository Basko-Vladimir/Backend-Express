...
export const checkPostRequestBody = [
	body("title")
		.trim().exists().withMessage("You didn't provide 'title' field")
		.isLength({max: 30}).withMessage("Name should be less than 30 chars"),
	body("shortDescription")
		.trim().exists().withMessage("You didn't provide 'shortDescription' field")
		.isLength({max: 100}).withMessage("Name should be less than 100 chars"),
	body("content")
		.trim().exists().withMessage("You didn't provide 'content' field")
		.isLength({max: 1000}).withMessage("Name should be less than 1000 chars"),
	body("bloggerId")
		.trim().exists().withMessage("You didn't provide 'bloggerId' field")
		.custom(value => {
